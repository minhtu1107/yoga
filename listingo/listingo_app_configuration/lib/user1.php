<?php
    ...
    //          return $user_meta;
                    ...
                    $data['nickname'] = $user_meta['nickname'][0];
                    $data['first_name'] = $user_meta['first_name'][0];
                    $data['last_name'] = $user_meta['last_name'][0];
                    $data['description'] = $user_meta['description'][0];
                    $data['comment_shortcuts'] = $user_meta['comment_shortcuts'][0];
                    $data['locale'] = $user_meta['locale'][0];
                    $data['verify_user'] = $user_meta['verify_user'][0];
                    $data['full_name'] = $user_meta['full_name'][0];
                    $data['activation_status'] = $user_meta['activation_status'][0];
                    $data['profile_photo'] = $user_meta['profile_photo'][0];
                    $data['profile_banner'] = $user_meta['profile_banner'][0];
                    $data['profile_appointment'] = $user_meta['profile_appointment'][0];
                    $data['profile_contact'] = $user_meta['profile_contact'][0];
                    $data['profile_hours'] = empty($user_meta['profile_hours'][0]) ? '' :$user_meta['profile_hours'][0];
                    $data['profile_service'] = $user_meta['profile_service'][0];
                    $data['profile_team'] = $user_meta['profile_team'][0];
                    $data['profile_gallery'] = $user_meta['profile_gallery'][0];
                    $data['profile_videos'] = $user_meta['profile_videos'][0];
                    $data['company_name'] = $user_meta['company_name'][0];
                    $data['zip'] = $user_meta['zip'][0];
                    $data['tag_line'] = $user_meta['tag_line'][0];
                    $data['phone'] = $user_meta['phone'][0];
                    $data['city'] = empty($user_meta['city'][0])?'':$user_meta['city'][0];
                    $data['country'] = empty($user_meta['country'][0])?'':$user_meta['country'][0];
                    $data['fax'] = $user_meta['fax'][0];
                    ...
                    $data['facebook'] = $user_meta['facebook'][0];
                    $data['twitter'] = $user_meta['twitter'][0];
                    $data['linkedin'] = $user_meta['linkedin'][0];
                    $data['pinterest'] = $user_meta['pinterest'][0];
                    $data['google_plus'] = empty($user_meta['google_plus'][0])?'': $user_meta['google_plus'][0];
                    $data['instagram'] = $user_meta['instagram'][0];
                    $data['tumblr'] = $user_meta['tumblr'][0];
                    $data['skype'] = $user_meta['skype'][0];
                    $data['last_update'] = $user_meta['last_update'][0];
                    $data['phone_number'] = $user_meta['phone_number'][0];
                    $data['email'] = $user_meta['email'][0];
                    
                    if( is_int($user_meta['category'][0])){
                        $data['category'] = get_cat_name($user_meta['category'][0]);  
                    }else{
                        $data['category'] = $user_meta['category'][0];    
                    }
                    if( !empty($user_meta['sub_category'][0])){
                    $data['sub_category'] = unserialize( $user_meta['sub_category'][0] ) ? unserialize($user_meta['sub_category'][0]) :array();
                    }else{
        ...
        $data['professional_statements'] = empty($user_meta['professional_statements'][0])?'':$user_meta['professional_statements'][0];
            ...
            'listingo_get_media_filter', listingo_get_user_banner(array('width' => 0, 'height' => 0), $user->data->ID), array('width' => 1920, 'height' => 380) //size width,height
                    ...
                    $data['user_banner'] = $user_meta['user_banner'][0];
