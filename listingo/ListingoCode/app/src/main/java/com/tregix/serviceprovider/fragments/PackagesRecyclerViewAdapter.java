...
public class PackagesRecyclerViewAdapter extends RecyclerView.Adapter<PackagesRecyclerViewAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        String sub = SharedPreferenceUtil.getStringValue(holder.title.getContext(),"sub_id");

        if (!sub.isEmpty() && (Integer.parseInt(sub) == holder.mItem.getId())){
                holder.currentPkg.setChecked(true);
        }else{
            holder.currentPkg.setVisibility(View.GONE);
        }

        if(mValues.get(position).getMetaData() != null && mValues.get(position).getMetaData().size() >=2) {
            holder.limit.setText(" "+ holder.limit.getContext().getString(R.string.for_pkg)+" " +
                    Html.fromHtml(mValues.get(position).getMetaData().get(1).getValue().toString())
                    +" "+ holder.limit.getContext().getString(R.string.days));
        }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View view) {
            ...
            currentPkg = view.findViewById(R.id.current_pkg);
        }
